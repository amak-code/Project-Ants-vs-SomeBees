...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            # Check if there IS a local ant
            if self.ant is None:
                valid = True
            else:
                # Check if local ant can contain the new insect
                if self.ant.can_contain(insect):
                    insect.place = self
                    self.ant.ant = insect
                    return
                # Check if new insect can contain local ant
                elif insect.can_contain(self.ant):
                    insect.ant = self.ant
                    valid = True

                # If neither will happen, raise exception error

            assert valid is True, 'Two ants in {0}'.format(self)
            self.ant = insect
        else:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if isinstance(insect, QueenAnt) and not insect.impostor:
                return
            if not insect.container or insect.ant is None:
                self.ant = None
            else:
                self.ant = insect.ant
...
class FireAnt(Ant):
    ...
    def damage_all_bees(self):
        print("All bees will take damage by fire ant!")
...
class QueenPlace:
    ...
    def bees(self):
        #Sasha's code
        #A QueenPlace has a bees property method that evaluates to the list of 
        # all bees that are either in the original colony.queen location or the place of the QueenAnt.
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        #Sasha's code
        #if the first queen was created all other should die immediatly (armor = 0)
